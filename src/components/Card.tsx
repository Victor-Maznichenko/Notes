...
const Card:FC<IProps> = ({ card, changeCard, to }) => {
    const dispatch = useStoreDispatch();
    const [currentTitle, setCurrentTitle] = useState(card.title);
    const [activeColor, setActiveColor] = useState(card.activeColor);
    const сontainsTitle = currentTitle.trim() !== '';
    const [showEditor, setShowEditor] = useState(!сontainsTitle);
        ...
        <div className={`hover:text-inherit text-inherit w-[25rem] rounded-2xl text-center ring-1 ring-inset px-3 py-3 h-52 relative ${activeColor}`}>
            {
                showEditor ?
                    <div className="flex justify-between">
                        <div>
                            ...
                            <ul className='grid grid-cols-4 w-24 h-24 gap-1 cursor-pointer'>
                                {
                                    card.aviableColors.map((color, index) => (
                                        <li key={index} className={`${color} ${color === card.activeColor ? 'border-green-800' : ''} border-solid border border-white`} onClick={() => (setActiveColor(color))}></li>
                                    ))
                                }
                            </ul>
                        </div>
                        <div className='flex flex-col items-start justify-between'>
                            <div className='text-left'>
                                ...
                                <input value={currentTitle} onChange={changeCurrentTitle} className={`peer m-0 block h-[30px] w-full rounded border border-solid border-neutral-300 bg-opacity-40 bg-black placeholder:text-transparent focus:border-primary focus:outline-none peer-focus:text-primary dark:focus:border-primary dark:peer-focus:text-primary px-1 ${!сontainsTitle ? 'border-red-600 color-red-300' : ''}`} type="text" />
                            </div>
                    ...
                    :
                    <>
                        <div className="absolute z-10 top-3 right-3 flex items-center">
                            <button type="button" className={`flex items-center mr-2 content-center rounded-md bg-red-600 p-1 shadow-sm`}>
                        ...
                        <Link to={to} className="flex w-full items-center h-full justify-center ">
